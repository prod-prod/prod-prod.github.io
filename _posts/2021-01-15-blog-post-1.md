---
title: 'Short story long: Hello world, C++. Part 1: compilation'
date: 2021-01-15
...
Every person who writes code some day wrote a hello world programm. Usually it is copying the text in any editor, save, compile and run. In this post we will have a deeper look in compilation and execution stages and undertsnd what hellens, once you press enter for compilation and execution.
...
And to build it, using `clang++` compiler, you need to run:
```bash
...
We can see phases by running command
```bash
...
Have you ever heard about [RVO](https://en.wikipedia.org/wiki/Copy_elision)? It happens at this stage together with many other optimizations.

The optimization happens in 2 steps:
1. Analysis
2. Optimization

Examples of optimizers for GCC, for example can be found here: https://gcc.gnu.org/wiki/MiddleEnd
...
If you want to better understand the output, run:
...
Important to say: every exacutable has a starting point in the memory.
